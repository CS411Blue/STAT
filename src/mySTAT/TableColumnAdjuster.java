...
public class TableColumnAdjuster implements PropertyChangeListener, TableModelListener
{
	...
	public void adjustColumn(final int column)
	{
		...
		int columnHeaderWidth = getColumnHeaderWidth( column );
                //if (columnHeaderWidth > 120)
                //    columnHeaderWidth = 120;
		int columnDataWidth   = getColumnDataWidth( column );
                //if (columnDataWidth > 120)
                //    columnDataWidth = 120;
		int preferredWidth = Math.max(columnHeaderWidth, columnDataWidth);

		updateTableColumn(column, preferredWidth);
	}
        public void copyColumns(JTable copythis)
        {
                TableColumnModel tcm = copythis.getColumnModel();

		for (int i = 0; i < tcm.getColumnCount(); i++)
		{
			TableColumn tableColumn = copythis.getColumnModel().getColumn(i);
